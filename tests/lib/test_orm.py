...
@pytest.mark.parametrize("query, expected_names", [
    ...
    ({"gender": "male", "age": 30}, {"Charlie", "Dave"}), # Test finding all elements with gender equal to male and age equal to 30
    ({"name": ".*a.*"}, {"Charlie", "Dave"}), # Test finding all elements with name containing the letter "a"
    ({"name": re.compile(r".*z.*", re.IGNORECASE)}, set()), # Test finding all elements with name containing the letter "z"
    ({"name": ".*a.*|.*e.*"}, {"Alice", "Charlie", "Dave"}), # Test finding all elements with name containing the letter "a" or "e"
    ({"name": re.compile(r".*ie$", re.IGNORECASE)}, {"Charlie"}), # Test finding all elements with name ending with "ie"
    ({"name": ""}, {"Alice", "Bob", "Charlie", "Dave"}), # Test finding all elements with an empty name (should return all elements)
    ({}, {"Alice", "Bob", "Charlie", "Dave"}), # Test finding all elements with no params (should return all elements)
    ({"age": 100}, set()), # Test finding all elements with age equal to 100 (should return an empty set)
    ({"name": "^A.*"}, {"Alice"}), # Test finding all elements with name starting with "A"
    ({"name": ".*e$"}, {"Alice", "Charlie", "Dave"}), # Test finding all elements with name ending with "e"
    ({"name": "^A.*|.*e$"}, {"Alice", "Dave", "Charlie"}), # Test finding all elements with name starting with "A" or ending with "e"
    ])
...
@pytest.mark.parametrize("query, expected_result", [
    ({"name": "Bob"}, {'name': 'Bob', "age": 40, "gender": "male"}),  # Test finding the element with name equal to "Bob"
    ({"name": "Alice", "age": 25}, {'age': 25, 'gender': 'female', 'name': 'Alice'}),  # Test finding the element with name equal to "Alice" and age equal to 25
    ({"name": ".*a.*"}, BaseMultipleFound),  # Test finding the element with name equal to "Charlie" (not unique)
    ({"name": "Eve"}, BaseNotFound),  # Test finding the element with name equal to "Eve" (not found)
    ({"age": 20}, BaseNotFound),  # Test finding the element with age equal to 20 (not found)
    ({"email": "alice@example.com"}, KeyError),  # Test calling the function with a key that doesn't exist in the collection
    ({"age__lt": 30}, {'age': 25, 'gender': 'female', 'name': 'Alice'}), # Test finding the element with age less than 30
    ({"gender__in": ["female", "nonbinary"]}, {'age': 25, 'gender': 'female', 'name': 'Alice'}), # Test finding the element with gender in ["female", "nonbinary"]
    ({"name__startswith": "C"}, {'name': 'Charlie', "age": 30, "gender": "male"}), # Test finding the element with name starting with "C"
    ({"gender__not": "male"}, {'age': 25, 'gender': 'female', 'name': 'Alice'}), # Test finding the element with gender not equal to "male"
    ({"name__endswith": "b"}, {"name": "Bob", "age": 40, "gender": "male"}), # Test finding all elements with name ending with "e"
    ({"age__lte": 25}, {"name": "Alice", "age": 25, "gender": "female"}), # Test finding all elements with age less than or equal to 30
    ({"age__gte": 40}, {"name": "Bob", "age": 40, "gender": "male"}), # Test finding all elements with age less than or equal to 30
    ({"age__gt": 39}, {"name": "Bob", "age": 40, "gender": "male"}), # Test finding all elements with age less than or equal to 30
    ({"age__nin": [25, 30]},  {"name": "Bob", "age": 40, "gender": "male"}), # Test finding all elements with age not equal to 25 or 30
    ({"name__contains": "v"}, {"name": "Dave", "age": 30, "gender": "male"}), # Test finding all elements with name containing the letter "v"
    ({"name__contains": "i"}, BaseMultipleFound),  # Test finding all elements with name containing the letter "i"
...
def test_orm_collection_group_by():
    ...
    assert grouped_lst == {'a': ['apple'],  'b': ['banana', 'bouger'], 'o': ['orange'], 'p': ['pear']}
...
def test_distinct():
    ...
    assert all(person.name in {"John", "Jane", "Bob", "Alice"} for person in distinct_persons)

    # Test with two fields
    distinct_persons = persons.distinct("name", "age")
    assert len(distinct_persons) == 4
    assert all(
        (person.name, person.age) in {("John", 25), ("Jane", 30), ("Bob", 25), ("Alice", 35)}
